class Solution:
    def numberToWords(self, num: int) -> str:
        ...
        for i in range(0, len(num), 3):
            ...
            if x > 0: section.append(o[x-1])
            if x > 0: section.append(p[0])
            if y > 0: 
                if y == 1:
                    section.append(d[z])
                else:
            ...
            if y != 1 and z > 0: section.append(o[z-1])
            if i > 0 and any([x,y,z]): section.append(p[i//3])
            ans.appendleft(' '.join(section))
            
        return ' '.join([a for a in ans if a != '']).strip()